...
//----------------------------------------
//-- Message MISSION_CLEAR_ALL pack,encode routines, for sending
//----------------------------------------
...
{
    fmav_mission_clear_all_t* _payload = (fmav_mission_clear_all_t*)_msg->payload;
    ...
    _msg->sysid = sysid;
    _msg->compid = compid;
    _msg->msgid = FASTMAVLINK_MSG_ID_MISSION_CLEAR_ALL;
    _msg->target_sysid = target_system;
    _msg->target_compid = target_component;
    _msg->crc_extra = FASTMAVLINK_MSG_MISSION_CLEAR_ALL_CRCEXTRA;
    _msg->payload_max_len = FASTMAVLINK_MSG_MISSION_CLEAR_ALL_PAYLOAD_LEN_MAX;
...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t fmav_msg_mission_clear_all_pack_to_frame_buf(
    ...
    fmav_status_t* _status)
{
    fmav_mission_clear_all_t* _payload = (fmav_mission_clear_all_t*)(&_buf[FASTMAVLINK_HEADER_V2_LEN]);
    ...
    _buf[5] = sysid;
    _buf[6] = compid;
    _buf[7] = (uint8_t)FASTMAVLINK_MSG_ID_MISSION_CLEAR_ALL;
    _buf[8] = ((uint32_t)FASTMAVLINK_MSG_ID_MISSION_CLEAR_ALL >> 8);
    _buf[9] = ((uint32_t)FASTMAVLINK_MSG_ID_MISSION_CLEAR_ALL >> 16);
...
//----------------------------------------
//-- Message MISSION_CLEAR_ALL decode routines, for receiving
//----------------------------------------
...
FASTMAVLINK_FUNCTION_DECORATOR void fmav_msg_mission_clear_all_decode(fmav_mission_clear_all_t* payload, const fmav_message_t* msg)
{
    ...
    if (msg->len < FASTMAVLINK_MSG_MISSION_CLEAR_ALL_PAYLOAD_LEN_MAX) {
        ...
        memset(&(((uint8_t*)payload)[msg->len]), 0, FASTMAVLINK_MSG_MISSION_CLEAR_ALL_PAYLOAD_LEN_MAX - msg->len);
    } else {
...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t mavlink_msg_mission_clear_all_encode(
    uint8_t sysid,
    uint8_t compid,
    mavlink_message_t* _msg,
    const mavlink_mission_clear_all_t* _payload)
{
