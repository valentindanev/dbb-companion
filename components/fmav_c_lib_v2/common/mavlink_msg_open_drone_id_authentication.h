...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t mavlink_msg_open_drone_id_authentication_encode(
    uint8_t sysid,
    uint8_t compid,
    mavlink_message_t* _msg,
    const mavlink_open_drone_id_authentication_t* _payload)
{
    return mavlink_msg_open_drone_id_authentication_pack(
        sysid,
        compid,
        _msg,
        _payload->target_system, _payload->target_component, _payload->id_or_mac, _payload->authentication_type, _payload->data_page, _payload->last_page_index, _payload->length, _payload->timestamp, _payload->authentication_data);
