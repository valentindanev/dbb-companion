...
//----------------------------------------
//-- Message ACTUATOR_OUTPUT_STATUS pack,encode routines, for sending
//----------------------------------------
...
{
    fmav_actuator_output_status_t* _payload = (fmav_actuator_output_status_t*)_msg->payload;
    ...
    _msg->sysid = sysid;
    _msg->compid = compid;
    _msg->msgid = FASTMAVLINK_MSG_ID_ACTUATOR_OUTPUT_STATUS;
    _msg->target_sysid = 0;
    _msg->target_compid = 0;
    _msg->crc_extra = FASTMAVLINK_MSG_ACTUATOR_OUTPUT_STATUS_CRCEXTRA;
    _msg->payload_max_len = FASTMAVLINK_MSG_ACTUATOR_OUTPUT_STATUS_PAYLOAD_LEN_MAX;
...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t fmav_msg_actuator_output_status_pack_to_frame_buf(
    ...
    fmav_status_t* _status)
{
    fmav_actuator_output_status_t* _payload = (fmav_actuator_output_status_t*)(&_buf[FASTMAVLINK_HEADER_V2_LEN]);
    ...
    _buf[5] = sysid;
    _buf[6] = compid;
    _buf[7] = (uint8_t)FASTMAVLINK_MSG_ID_ACTUATOR_OUTPUT_STATUS;
    _buf[8] = ((uint32_t)FASTMAVLINK_MSG_ID_ACTUATOR_OUTPUT_STATUS >> 8);
    _buf[9] = ((uint32_t)FASTMAVLINK_MSG_ID_ACTUATOR_OUTPUT_STATUS >> 16);
...
//----------------------------------------
//-- Message ACTUATOR_OUTPUT_STATUS decode routines, for receiving
//----------------------------------------
...
FASTMAVLINK_FUNCTION_DECORATOR void fmav_msg_actuator_output_status_decode(fmav_actuator_output_status_t* payload, const fmav_message_t* msg)
{
    ...
    if (msg->len < FASTMAVLINK_MSG_ACTUATOR_OUTPUT_STATUS_PAYLOAD_LEN_MAX) {
        ...
        memset(&(((uint8_t*)payload)[msg->len]), 0, FASTMAVLINK_MSG_ACTUATOR_OUTPUT_STATUS_PAYLOAD_LEN_MAX - msg->len);
    } else {
...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t mavlink_msg_actuator_output_status_encode(
    uint8_t sysid,
    uint8_t compid,
    mavlink_message_t* _msg,
    const mavlink_actuator_output_status_t* _payload)
{
    return mavlink_msg_actuator_output_status_pack(
        sysid,
        compid,
        _msg,
        _payload->time_usec, _payload->active, _payload->actuator);
