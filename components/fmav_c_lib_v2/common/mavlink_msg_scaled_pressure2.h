...
//----------------------------------------
//-- Message SCALED_PRESSURE2 pack,encode routines, for sending
//----------------------------------------
...
{
    fmav_scaled_pressure2_t* _payload = (fmav_scaled_pressure2_t*)_msg->payload;
    ...
    _msg->sysid = sysid;
    _msg->compid = compid;
    _msg->msgid = FASTMAVLINK_MSG_ID_SCALED_PRESSURE2;
    _msg->target_sysid = 0;
    _msg->target_compid = 0;
    _msg->crc_extra = FASTMAVLINK_MSG_SCALED_PRESSURE2_CRCEXTRA;
    _msg->payload_max_len = FASTMAVLINK_MSG_SCALED_PRESSURE2_PAYLOAD_LEN_MAX;
...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t fmav_msg_scaled_pressure2_pack_to_frame_buf(
    ...
    fmav_status_t* _status)
{
    fmav_scaled_pressure2_t* _payload = (fmav_scaled_pressure2_t*)(&_buf[FASTMAVLINK_HEADER_V2_LEN]);
    ...
    _buf[5] = sysid;
    _buf[6] = compid;
    _buf[7] = (uint8_t)FASTMAVLINK_MSG_ID_SCALED_PRESSURE2;
    _buf[8] = ((uint32_t)FASTMAVLINK_MSG_ID_SCALED_PRESSURE2 >> 8);
    _buf[9] = ((uint32_t)FASTMAVLINK_MSG_ID_SCALED_PRESSURE2 >> 16);
...
//----------------------------------------
//-- Message SCALED_PRESSURE2 decode routines, for receiving
//----------------------------------------
...
FASTMAVLINK_FUNCTION_DECORATOR void fmav_msg_scaled_pressure2_decode(fmav_scaled_pressure2_t* payload, const fmav_message_t* msg)
{
    ...
    if (msg->len < FASTMAVLINK_MSG_SCALED_PRESSURE2_PAYLOAD_LEN_MAX) {
        memcpy(payload, msg->payload, msg->len);
        // ensure that returned payload is zero-filled
        memset(&(((uint8_t*)payload)[msg->len]), 0, FASTMAVLINK_MSG_SCALED_PRESSURE2_PAYLOAD_LEN_MAX - msg->len);
    } else {
        // note: msg->len can be larger than PAYLOAD_LEN_MAX if the message has unknown extensions
        memcpy(payload, msg->payload, FASTMAVLINK_MSG_SCALED_PRESSURE2_PAYLOAD_LEN_MAX);
    }
#else
...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t mavlink_msg_scaled_pressure2_encode(
    uint8_t sysid,
    uint8_t compid,
    mavlink_message_t* _msg,
    const mavlink_scaled_pressure2_t* _payload)
{
