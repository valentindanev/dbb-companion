...
//----------------------------------------
//-- Message HOME_POSITION pack,encode routines, for sending
//----------------------------------------
...
{
    fmav_home_position_t* _payload = (fmav_home_position_t*)_msg->payload;
    ...
    _msg->sysid = sysid;
    _msg->compid = compid;
    _msg->msgid = FASTMAVLINK_MSG_ID_HOME_POSITION;
    _msg->target_sysid = 0;
    _msg->target_compid = 0;
    _msg->crc_extra = FASTMAVLINK_MSG_HOME_POSITION_CRCEXTRA;
    _msg->payload_max_len = FASTMAVLINK_MSG_HOME_POSITION_PAYLOAD_LEN_MAX;
...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t fmav_msg_home_position_pack_to_frame_buf(
    ...
    fmav_status_t* _status)
{
    fmav_home_position_t* _payload = (fmav_home_position_t*)(&_buf[FASTMAVLINK_HEADER_V2_LEN]);
    ...
    _buf[5] = sysid;
    _buf[6] = compid;
    _buf[7] = (uint8_t)FASTMAVLINK_MSG_ID_HOME_POSITION;
    _buf[8] = ((uint32_t)FASTMAVLINK_MSG_ID_HOME_POSITION >> 8);
    _buf[9] = ((uint32_t)FASTMAVLINK_MSG_ID_HOME_POSITION >> 16);
...
//----------------------------------------
//-- Message HOME_POSITION decode routines, for receiving
//----------------------------------------
...
FASTMAVLINK_FUNCTION_DECORATOR void fmav_msg_home_position_decode(fmav_home_position_t* payload, const fmav_message_t* msg)
{
    ...
    if (msg->len < FASTMAVLINK_MSG_HOME_POSITION_PAYLOAD_LEN_MAX) {
        memcpy(payload, msg->payload, msg->len);
        // ensure that returned payload is zero-filled
        memset(&(((uint8_t*)payload)[msg->len]), 0, FASTMAVLINK_MSG_HOME_POSITION_PAYLOAD_LEN_MAX - msg->len);
    } else {
        // note: msg->len can be larger than PAYLOAD_LEN_MAX if the message has unknown extensions
        memcpy(payload, msg->payload, FASTMAVLINK_MSG_HOME_POSITION_PAYLOAD_LEN_MAX);
    }
#else
...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t mavlink_msg_home_position_encode(
    uint8_t sysid,
    uint8_t compid,
    mavlink_message_t* _msg,
    const mavlink_home_position_t* _payload)
{
    return mavlink_msg_home_position_pack(
        sysid,
        compid,
        _msg,
        _payload->latitude, _payload->longitude, _payload->altitude, _payload->x, _payload->y, _payload->z, _payload->q, _payload->approach_x, _payload->approach_y, _payload->approach_z, _payload->time_usec);
