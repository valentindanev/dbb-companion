...
//----------------------------------------
//-- Message VICON_POSITION_ESTIMATE pack,encode routines, for sending
//----------------------------------------
    ...
    fmav_status_t* _status)
{
    ...
    _msg->payload_max_len = FASTMAVLINK_MSG_VICON_POSITION_ESTIMATE_PAYLOAD_LEN_MAX;
...
//----------------------------------------
//-- Message VICON_POSITION_ESTIMATE decode routines, for receiving
//----------------------------------------
...
FASTMAVLINK_FUNCTION_DECORATOR void fmav_msg_vicon_position_estimate_decode(fmav_vicon_position_estimate_t* payload, const fmav_message_t* msg)
{
    ...
    if (msg->len < FASTMAVLINK_MSG_VICON_POSITION_ESTIMATE_PAYLOAD_LEN_MAX) {
        ...
        memset(&(((uint8_t*)payload)[msg->len]), 0, FASTMAVLINK_MSG_VICON_POSITION_ESTIMATE_PAYLOAD_LEN_MAX - msg->len);
    } else {
...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t mavlink_msg_vicon_position_estimate_encode(
    uint8_t sysid,
    uint8_t compid,
    mavlink_message_t* _msg,
    const mavlink_vicon_position_estimate_t* _payload)
{
