...
//----------------------------------------
//-- Message SCALED_PRESSURE3 pack,encode routines, for sending
//----------------------------------------
    ...
    fmav_status_t* _status)
{
    ...
    _msg->payload_max_len = FASTMAVLINK_MSG_SCALED_PRESSURE3_PAYLOAD_LEN_MAX;
...
//----------------------------------------
//-- Message SCALED_PRESSURE3 decode routines, for receiving
//----------------------------------------
...
FASTMAVLINK_FUNCTION_DECORATOR void fmav_msg_scaled_pressure3_decode(fmav_scaled_pressure3_t* payload, const fmav_message_t* msg)
{
    ...
    if (msg->len < FASTMAVLINK_MSG_SCALED_PRESSURE3_PAYLOAD_LEN_MAX) {
        ...
        memset(&(((uint8_t*)payload)[msg->len]), 0, FASTMAVLINK_MSG_SCALED_PRESSURE3_PAYLOAD_LEN_MAX - msg->len);
    } else {
...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t mavlink_msg_scaled_pressure3_encode(
    uint8_t sysid,
    uint8_t compid,
    mavlink_message_t* _msg,
    const mavlink_scaled_pressure3_t* _payload)
{
    return mavlink_msg_scaled_pressure3_pack(
        sysid,
        compid,
        _msg,
        _payload->time_boot_ms, _payload->press_abs, _payload->press_diff, _payload->temperature, _payload->temperature_press_diff);
