...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t mavlink_msg_open_drone_id_basic_id_encode(
    uint8_t sysid,
    uint8_t compid,
    mavlink_message_t* _msg,
    const mavlink_open_drone_id_basic_id_t* _payload)
{
    return mavlink_msg_open_drone_id_basic_id_pack(
        sysid,
        compid,
        _msg,
        _payload->target_system, _payload->target_component, _payload->id_or_mac, _payload->id_type, _payload->ua_type, _payload->uas_id);
