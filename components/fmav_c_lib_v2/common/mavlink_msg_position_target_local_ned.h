...
//----------------------------------------
//-- Message POSITION_TARGET_LOCAL_NED pack,encode routines, for sending
//----------------------------------------
...
{
    fmav_position_target_local_ned_t* _payload = (fmav_position_target_local_ned_t*)_msg->payload;
    ...
    _msg->sysid = sysid;
    _msg->compid = compid;
    _msg->msgid = FASTMAVLINK_MSG_ID_POSITION_TARGET_LOCAL_NED;
    _msg->target_sysid = 0;
    _msg->target_compid = 0;
    _msg->crc_extra = FASTMAVLINK_MSG_POSITION_TARGET_LOCAL_NED_CRCEXTRA;
    _msg->payload_max_len = FASTMAVLINK_MSG_POSITION_TARGET_LOCAL_NED_PAYLOAD_LEN_MAX;
...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t fmav_msg_position_target_local_ned_pack_to_frame_buf(
    ...
    fmav_status_t* _status)
{
    fmav_position_target_local_ned_t* _payload = (fmav_position_target_local_ned_t*)(&_buf[FASTMAVLINK_HEADER_V2_LEN]);
    ...
    _buf[5] = sysid;
    _buf[6] = compid;
    _buf[7] = (uint8_t)FASTMAVLINK_MSG_ID_POSITION_TARGET_LOCAL_NED;
    _buf[8] = ((uint32_t)FASTMAVLINK_MSG_ID_POSITION_TARGET_LOCAL_NED >> 8);
    _buf[9] = ((uint32_t)FASTMAVLINK_MSG_ID_POSITION_TARGET_LOCAL_NED >> 16);
...
//----------------------------------------
//-- Message POSITION_TARGET_LOCAL_NED decode routines, for receiving
//----------------------------------------
...
FASTMAVLINK_FUNCTION_DECORATOR void fmav_msg_position_target_local_ned_decode(fmav_position_target_local_ned_t* payload, const fmav_message_t* msg)
{
    ...
    if (msg->len < FASTMAVLINK_MSG_POSITION_TARGET_LOCAL_NED_PAYLOAD_LEN_MAX) {
        ...
        memset(&(((uint8_t*)payload)[msg->len]), 0, FASTMAVLINK_MSG_POSITION_TARGET_LOCAL_NED_PAYLOAD_LEN_MAX - msg->len);
    } else {
...
FASTMAVLINK_FUNCTION_DECORATOR uint16_t mavlink_msg_position_target_local_ned_encode(
    uint8_t sysid,
    uint8_t compid,
    mavlink_message_t* _msg,
    const mavlink_position_target_local_ned_t* _payload)
{
